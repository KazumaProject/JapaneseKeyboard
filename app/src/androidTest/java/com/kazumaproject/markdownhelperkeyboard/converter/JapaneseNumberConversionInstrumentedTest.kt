...
class JapaneseNumberConversionInstrumentedTest {
    ...
    @Test
    fun ordinaryHomophonesAreNotPromotedToQuantities() = runBlocking {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val entryPoint = EntryPointAccessors.fromApplication(context.applicationContext, KanaKanjiEngineEntryPoint::class.java)
        for ((input, expected) in mapOf("はっけんだけ" to "発見だけ", "ごかいだけ" to "誤解だけ",
                "いつかだけ" to "いつかだけ", "にほんだけ" to "日本だけ")) {
            val candidates = entryPoint.kanaKanjiEngine().convertOriginal(input, entryPoint.userDictionaryRepository())
            assertEquals(input, expected, candidates.first().string)
        }
    }
