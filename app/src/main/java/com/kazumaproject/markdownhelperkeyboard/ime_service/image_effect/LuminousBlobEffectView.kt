...
class LuminousBlobEffectView @JvmOverloads constructor(
    context: Context,
    ...
    private val inputInjector = LuminousBlobInputInjector(inputQueue)

    @VisibleForTesting
    internal var rendererFactory: LuminousBlobRendererFactory =
        LuminousBlobRendererFactory { queue, callback ->
            LuminousBlobRenderer(
                inputQueue = queue,
                callback = callback
            )
        }

    private var renderer: LuminousBlobRendererController? = null
    private var effectEnabled = false
    private var currentSettings = LuminousBlobSettings.Disabled
    ...
    fun configure(
        ...
        currentSettings = LuminousBlobSettings(
            enabled = enabled,
            colorMode = colorMode,
            fixedColor = LuminousBlobSettings.withoutTransparentAlpha(fixedColor),
    ...
    private fun ensureRenderer(): LuminousBlobRendererController {
        renderer?.let { return it }
        return rendererFactory.create(
            inputQueue,
            LuminousBlobRendererCallback { reason, throwable ->
                Timber.w(throwable, "Keyboard luminous blob effect disabled: %s", reason)
