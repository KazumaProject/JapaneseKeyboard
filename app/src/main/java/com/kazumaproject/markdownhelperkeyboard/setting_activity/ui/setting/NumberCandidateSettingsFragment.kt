...
class NumberUnitEditorFragment : Fragment() {
    ...
    private fun candidates(unit: CustomNumberUnit, input: String): String = if (!CustomNumberUnit.validReading(input)) "" else engine.getCandidatesEnglishKana(input,
        PredictionConfig(japaneseNumberCandidatesEnabled = true, numberCandidateOrder = NumberCandidateOrder.fromPreference(AppPreference.number_candidate_order_preference), numberCandidateConfig = NumberCandidateConfig(units = listOf(unit.copy(enabled = true)))))
        .filter { it.generatedNumber && it.number?.customUnit?.id == unit.id }.joinToString("・") { it.string }
