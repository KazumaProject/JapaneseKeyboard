...
Dictionary provenance is the rule for lexical preservation; there is no fixed list of word exceptions. All bundled dictionary entries are audited, including 一時 (いっとき), 万人 (ばんにん), and the explicitly registered single-kanji reading いっ → 一. This does not authorize generating 1 from いっ. Dictionary counter compositions such as 一本 / 六個 / 八回 retain their own segment evidence without expanding the standalone generation grammar. Likewise, a different valid reading of ごはん may produce 5版 or 5班: a test must reject the incorrect numeral interpretation, not every digit-containing lexical alternative.

Persisted lexical candidates reconstruct exact reading/output evidence from loaded dictionaries before filtering. Explicit user entries justify only their own output; adjoining automatic numerals still need a valid numeric interval and boundary. Long direct digit strings and leading zeros remain literal text even beyond Long's range.
