...
class LiquidRippleSimulationGridResolverTest {

    private val resolver = LiquidRippleSimulationGridResolver()
    ...
    fun gridKeepsAspectRatioEvenAlignmentAndMinimums() {
        val grid = resolver.resolve(360, 1200, 0, KeyboardTouchEffectQuality.ULTRA)
        ...
        assertEquals(360f / 1200f, grid.width.toFloat() / grid.height, 0.08f)

        val invalid = resolver.resolve(-1, 0, 0, KeyboardTouchEffectQuality.EXTREME)
        assertEquals(32, invalid.width)
        assertEquals(32, invalid.height)
    }

    private fun resolve(quality: String): LiquidRippleSimulationGrid {
        return resolver.resolve(3000, 1200, 0, quality)
    }

    private fun longSide(grid: LiquidRippleSimulationGrid): Int = maxOf(grid.width, grid.height)
}
